 ...
 *
 * Mỗi giá trị Enum tương ứng với 1 loại quái trong game, chứa đầy đủ thông số mặc định.
 * Khi thêm quái mới, chỉ cần thêm 1 dòng Enum ở đây là xong, không cần sửa ở nơi nào khác.
 *
 * TiledAshleyConfigurator sẽ đọc EnemyDef làm GIÁ TRỊ MẶC ĐỊNH, sau đó ưu tiên đọc
 * thêm Custom Properties trong Tiled để ghi đè (Override) nếu cần tùy chỉnh cho 1 ô cụ thể.
 *
 * Đơn vị đo lường: "world unit" (1 unit = 1 ô gạch 16px, đơn vị chuẩn của Box2D).
 */
public enum EnemyDef {

    // ─────────────────────────────────────────────────────────────────────────
    //  id             sprW  sprH  bodyR  speed  wander  aggro  deAggro  leash
    // ─────────────────────────────────────────────────────────────────────────
    BAT     ("bat",     16,   16,  0.3f,  1.5f,   3f,     4f,    6f,     8f),
    SLIME   ("slime",   16,   16,  0.35f, 0.8f,   2f,     3f,    5f,     6f),
    SKELETON("skeleton",16,   16,  0.4f,  1.2f,   2f,     5f,    7f,    10f);
    ...
    EnemyDef(String id, int spriteW, int spriteH, float bodyRadius, float speed,
             float wanderRadius, float aggroRange, float deAggroRange, float leashRange) {
        this.id           = id;
     ...
     * Trả về BAT làm fallback nếu không tìm thấy, để game không crash khi đặt sai tên.
     */
